...
# 데이터 읽어오기
with client.read("output/arr_delay_count/part-r-00000", encoding="utf-8") \
        as reader:
    data = reader.read()
...
# year, month 컬럼을 int으로 변환
df['year'] = df['year'].astype("int")
df['month'] = df['month'].astype("int")

# print(df)
# 데이터 프레일을 year 기준으로 group
grouped_df = df.groupby("year")

# 시각화
fig, ax = plt.subplots()    # 여러개의 플롯을 겹치기위한 서브 플롯
